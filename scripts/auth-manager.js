#!/usr/bin/env node
/**
 * Manage pending client authorization requests with a signed admin proof.
 *
 * Usage:
...
var adminToken = process.env[tokenEnv];
if (!adminToken) {
...
var op;
if (action === 'next') {
    op = Authorization.nextAuthorizationRequest(adminToken, realm ? {realm: realm} : {}, options);
}
else {
    if (!requestId) {
    ...
    op = Authorization.decideAuthorizationRequest(adminToken, {
