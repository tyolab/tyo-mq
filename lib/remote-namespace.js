...
// This module implements the pluggable-namespace contract and is bundled
// as the built-in '/remote' plugin:
//
//   module.exports = function attach(io, options, context) -> api
//     io       — the socket.io server (call io.of('/name') yourself)
//     options  — the namespace's options block at attach time
//     context  — {name, logger, getOptions, getSettings}; getOptions()
//                returns the live options block, so configuration
//                hot-reloads without re-attaching
//
// Custom namespaces use the same contract via the `namespaces` setting —
// see lib/server.js attachConfiguredNamespaces.
module.exports = function attachRemoteNamespace(io, options, context) {
    options = options || {};
    ...
    // Live view of this namespace's options block so relay rules
    // hot-reload; falls back to the attach-time options when the caller
    // provides no context (a bare function is accepted for compatibility).
    var liveConfig = function () { return options; };
    if (context && typeof context.getOptions === 'function')
        liveConfig = context.getOptions;
    ...
    nsp.on('connection', function (socket) {
        ...
        socket.onAny(function (event) {
            ...
            if (rule.binary) {
                // frame: re-emitted to viewers as a binary Buffer. New agents
                // send the frame as a raw binary attachment (a Buffer, no
                // base64); older agents send { frame: <base64> }. Forward binary
                // verbatim; decode the legacy base64 shape. Either way viewers
                // receive a Buffer, so their handling is unchanged.
