...
function deliverNotifyPush(cfg, registry, realm, topic, msg, mode, opts) {
    ...
    try {
        ...
        // (per-field caps bound everything else well under it). Fidelity over
        // content: downgrade this one push to a contentless wake — the app
        // fetches the full message from the ring, buttons intact — rather
        // than ever sending a content payload with silently-dropped actions.
        if (mode === 'content' && msg && msg.actions && msg.actions.length &&
                JSON.stringify(payload).length > NOTIFY_PUSH_TOTAL_MAX) {
