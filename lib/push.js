...
function FcmTransport(opts) {
    ...
    this.timeoutMs = opts.timeoutMs || PUSH_HTTP_TIMEOUT_MS;
    this._sa = sa;
    this._tokenUrl = sa.token_uri || 'https://oauth2.googleapis.com/token';
    ...
    this._tokenPromise = null;   // in-flight mint (dedupes concurrent sends)
    // Optional logger for the token-free AUTH-failure warn (401/403). Injected in
    // tests; wired to server.logger at boot. Never receives token material.
...
FcmTransport.prototype._getAccessToken = function () {
    var self = this;
    if (self._token && Date.now() < self._token.expiresAtMs - FCM_TOKEN_REFRESH_MARGIN_MS)
        return Promise.resolve(self._token.value);
    if (self._tokenPromise) return self._tokenPromise;
    self._tokenPromise = self._mintAccessToken().then(
        function (tok) { self._tokenPromise = null; self._token = tok; return tok.value; },
        function (err) { self._tokenPromise = null; throw err; },
    );
    return self._tokenPromise;
};

// OAuth 2.0 JWT-bearer: sign an RS256 assertion with the SA private key and
// exchange it at the token endpoint for a short-lived access token.
FcmTransport.prototype._mintAccessToken = function () {
    var self = this;
    ...
    return simplePost(self._tokenUrl, {
        'content-type': 'application/x-www-form-urlencoded',
    }, body, self.timeoutMs).then(function (res) {
    ...
    return self._getAccessToken()
        .then(function (accessToken) {
            ...
            var url = self._fcmBaseUrl + '/v1/projects/' +
                encodeURIComponent(self._sa.project_id) + '/messages:send';
...
// Build an FcmTransport from env: TYO_MQ_PUSH_FCM_CREDENTIALS points at the
// service-account JSON (Firebase console -> Service accounts -> Generate new
// private key). Read + validated at boot so a misconfig fails loudly at config
// time, never per-send.
function createFcmTransportFromEnv(env) {
    ...
    try {
        ...
        throw new Error('TYO_MQ_PUSH_FCM_CREDENTIALS unreadable or not JSON: ' + credFile + ' (' + e.message + ')');
    }
    if (sa.type !== 'service_account')
        throw new Error('TYO_MQ_PUSH_FCM_CREDENTIALS must be a service_account JSON (type=service_account): ' + credFile);
    if (!sa.client_email || !sa.private_key || !sa.project_id)
        throw new Error('TYO_MQ_PUSH_FCM_CREDENTIALS must contain client_email, private_key and project_id: ' + credFile);
    return new FcmTransport({ serviceAccount: sa });
}

// Factory: select a transport implementation by name. 'null' (dev/test),
// 'unifiedpush' (self-hosted HTTP wake) and 'fcm' (FCM HTTP v1) are wired;
// apns remains a reserved seam that throws a clear "not implemented" so a
...
function createTransport(name, env) {
    ...
    switch (name) {
        ...
        case TRANSPORT_APNS:
            throw new Error("push transport '" + name + "' is not implemented yet (P4b wires 'null', 'unifiedpush' and 'fcm')");
        default:
