...
function ApnsTransport(opts) {
    ...
    this._lastNoTopicWarnMs = 0;    // throttle for the no-topic warn (see _warnNoTopic)
}
...
function TokenRegistry(opts) {
    ...
    // Optional global bounds (used by the PUBLIC notify registry; 0/unset keeps
    // the sealed/wake registry unbounded, since that one is gated by auth).
    this.maxIdentities = opts.maxIdentities || 0;   // 0 = unbounded identities/realm
    this.ttlMs = opts.ttlMs || 0;                   // 0 = no idle-expiry
    this._byRealm = new Map();   // realm -> Map(identity -> [endpoint])
    this._lastWake = new Map();  // realm -> Map(identity -> ts)
    this._touch = new Map();     // realm -> Map(identity -> ts)  (LRU + TTL)
    // Optional durable backing store (lib/notify-push-store.js). The PUBLIC
    // notify registry passes one so registrations SURVIVE A BROKER RESTART;
    // the auth-gated sealed/wake registry leaves it null (in-memory only).
    this.store = opts.store || null;
    if (this.store) this._hydrate();
}

// Persist helpers — best-effort; a store write must never break a live
// (un)register (the in-memory registry is authoritative during a run, the
// store is only re-read on the next boot).
TokenRegistry.prototype._storePut = function (realm, identity, ep) {
    try { this.store.put(realm, identity, ep); } catch (e) {}
};
TokenRegistry.prototype._storeDel = function (realm, identity, ep) {
    try { this.store.del(realm, identity, ep.transport, ep.token); } catch (e) {}
};

// Load persisted endpoints into memory on construction. Rows past the idle TTL
// are skipped (and swept from the store) rather than resurrected, so a restart
// never revives a long-dead device.
TokenRegistry.prototype._hydrate = function () {
    var self = this;
    var now = Date.now();
    var cutoff = this.ttlMs ? now - this.ttlMs : 0;
    var rows;
    try { rows = this.store.all(); }
    catch (e) { return; }
    (rows || []).forEach(function (row) {
        if (cutoff && (Number(row.added_at) || 0) < cutoff) return; // stale — skip
        var m = self._identityMap(row.realm, true);
        var list = m.get(row.identity);
        if (!list) { list = []; m.set(row.identity, list); }
        list.push({
            transport: row.transport,
            token: row.token,
            app_id: row.app_id != null ? row.app_id : null,
            env: row.env != null ? row.env : null,
            min_priority: (typeof row.min_priority === 'number') ? row.min_priority : null,
            added_at: Number(row.added_at) || 0,
            last_ok: null,
        });
        while (list.length > self.maxPerIdentity) list.shift();
        var tm = self._touchMap(row.realm, true);
        tm.delete(row.identity);
        tm.set(row.identity, Number(row.added_at) || now);
    });
    if (cutoff) { try { self.store.pruneOlderThan(cutoff); } catch (e) {} }
};

TokenRegistry.prototype._touchMap = function (realm, create) {
    var m = this._touch.get(realm);
    if (!m && create) { m = new Map(); this._touch.set(realm, m); }
    return m || null;
};

// Drop identities not touched within ttlMs (no-op unless ttlMs is set).
TokenRegistry.prototype._sweepExpired = function (realm, now) {
    if (!this.ttlMs) return;
    var tm = this._touchMap(realm, false);
    if (!tm) return;
    var m = this._identityMap(realm, false);
    var cutoff = now - this.ttlMs;
    tm.forEach(function (ts, identity) {
        if (ts < cutoff) {
            if (m) m.delete(identity);
            tm.delete(identity);
        }
    });
};
...
TokenRegistry.prototype.register = function (realm, identity, ep) {
    ...
    if (this.ttlMs) this._sweepExpired(realm, now);
    ...
    if (result) {
        result.added_at = now;
        if (ep.app_id !== undefined) result.app_id = ep.app_id;
        if (ep.env !== undefined) result.env = ep.env;
        ...
        while (list.length > this.maxPerIdentity) {
            var evictedEp = list.shift();   // evict oldest token for this identity
            if (this.store && evictedEp) this._storeDel(realm, identity, evictedEp);
        }
    }
    // LRU touch + global identity cap (both no-ops unless maxIdentities is set).
    var tm = this._touchMap(realm, true);
    tm.delete(identity);
    tm.set(identity, now);   // re-insert → most-recently-used at the tail
    if (this.maxIdentities) {
        while (m.size > this.maxIdentities) {
            var oldest = tm.keys().next().value; // least-recently-used at the head
            if (oldest === undefined || oldest === identity) break;
            var evictedList = m.get(oldest);
            m.delete(oldest);
            tm.delete(oldest);
            if (this.store && evictedList) {
                for (var ei = 0; ei < evictedList.length; ei++)
                    this._storeDel(realm, oldest, evictedList[ei]);
            }
        }
    }
    if (this.store) this._storePut(realm, identity, result);
    return result;
};
...
TokenRegistry.prototype.unregister = function (realm, identity, ep) {
    ...
    if (kept.length === 0) {
        m.delete(identity);
        var tm = this._touchMap(realm, false);
        if (tm) tm.delete(identity);
    }
    else m.set(identity, kept);
    if (this.store) this._storeDel(realm, identity, ep);
...
var NOTIFY_PUSH_TOTAL_MAX = 3500; // headroom under FCM's 4096-byte data limit
...
function buildNotifyPayload(msg, mode) {
    ...
    if (mode === 'content') {
        ...
        p.message = body;
        // Carry the message time (unix seconds) so the app renders WHEN the
        // notification was generated, not when the phone received it. Absent it,
        // the client falls back to receipt time (NotifyPayload).
        if (msg.time) p.time = String(msg.time);
        // Keep the total FCM `data` payload well under the 4KB limit.
        if (msg.title) p.title = String(msg.title).slice(0, 256);
        if (msg.priority) p.priority = String(msg.priority);
        if (msg.tags && msg.tags.length) p.tags = msg.tags.join(',').slice(0, 256);
        if (msg.click) p.click = String(msg.click).slice(0, 512);
        // Actions ride verbatim as a JSON string (FCM data values must be
        // strings). Never truncated here — when the total payload is over
        // budget, deliverNotifyPush downgrades the whole push to a wake
        // instead of shipping a content payload with broken/dropped buttons.
        if (msg.actions && msg.actions.length) p.actions = JSON.stringify(msg.actions);
    } else {
...
function deliverNotifyPush(cfg, registry, realm, topic, msg, mode, opts) {
    ...
    try {
        ...
        // Content payloads carrying actions can exceed FCM's 4KB data cap.
        // Fidelity over content: downgrade this one push to a contentless
        // wake — the app fetches the full message from the ring, buttons
        // intact — rather than ever sending a content payload with
        // silently-dropped actions. Measured in BYTES: FCM's limit is bytes,
        // and multibyte content near the publish body cap can be ~2x its
        // UTF-16 char count — a char-based check let such payloads through
        // to a guaranteed FCM rejection (no retry, no push at all).
        if (mode === 'content' && msg && msg.actions && msg.actions.length &&
                Buffer.byteLength(JSON.stringify(payload), 'utf8') > NOTIFY_PUSH_TOTAL_MAX) {
            payload = buildNotifyPayload(msg, 'wake');
        }
