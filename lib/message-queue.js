        ...
        io.sockets.on('connection', function(socket) {
            ...
            function sendSubscriptionMessage(eventStr, producer, consumer, consumerId) {
                ...
                if (producerInfo && producerInfo.id)
                    sendSubscriptionMessageWithConsumerInfo(producerInfo.id, eventStr, producer, consumer, consumerId);
            }

            // it seems the new updates weren't pushed to the remote repo
            function sendSubscriptionMessageWithConsumerInfo(id, eventStr, producer, consumer, consumerId) {
                var onSubscribeEvent = eventManager.toOnSubscribeEvent(producer);
                sendMessage(id, onSubscribeEvent, {name:consumer, id:consumerId, event:eventStr});
            }
