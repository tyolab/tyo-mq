...
// max_queued_per_realm quota. Optional `event` and `consumer` narrow the
// count (used by the per-recipient sealed-inbox quota, where event is
// 'sealed:<identity>' and consumer is the identity).
SQLiteStore.prototype.countQueued = function (realm, event, consumer) {
    this._purgeExpired();
    var sql = 'SELECT COUNT(*) AS n FROM messages WHERE realm = ?';
    var params = [String(realm || 'default')];
...
SQLiteStore.prototype.purgeRealm = function (realm) {
    var targetRealm = String(realm || '');
    var messages = this.db.prepare('DELETE FROM messages WHERE realm = ?').run(targetRealm);
    var dlq = this.db.prepare('DELETE FROM dlq WHERE realm = ?').run(targetRealm);
    return Promise.resolve(Number(messages.changes || 0) + Number(dlq.changes || 0));
};
