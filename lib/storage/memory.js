...
MemoryStore.prototype.countQueued = function (realm, event, consumer) {
    this._purgeExpired();
    var targetRealm = String(realm || 'default');
    var targetEvent = event === undefined || event === null ? null : String(event);
    var targetConsumer = consumer === undefined || consumer === null ? null : String(consumer);
    return Promise.resolve(this.messages.filter(function (entry) {
        if (entry.realm !== targetRealm)
            return false;
        if (targetEvent !== null && entry.event !== targetEvent)
            return false;
        if (targetConsumer !== null && String(entry.consumer || '') !== targetConsumer)
            return false;
        return true;
    }).length);
};
