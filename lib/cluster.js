...
// ── E2EE public-key directory (E2EE.md) ──────────────────────────────────────
// The realm key directory is shared cluster-wide so a KEY_LOOKUP on any node
// sees keys published on any other. Public keys only — never private material.

ClusterSync.prototype._e2eeKeysKey = function () {
    return this.prefix + ':e2eekeys';
};

ClusterSync.prototype._e2eeKeysChannel = function () {
    return this.prefix + ':e2eekeys:changed';
};

/**
 * Fetch the shared key-directory document
 * (map of realm → identity → [key entries]). Returns {} when empty.
 */
ClusterSync.prototype.getE2eeKeys = function () {
    return this.client.sendCommand(['GET', this._e2eeKeysKey()]).then(function (raw) {
        if (!raw)
            return {};
        try {
            return JSON.parse(raw) || {};
...
ClusterSync.prototype.putE2eeKeys = function (doc) {
    var self = this;
    return this.client.sendCommand(['SET', this._e2eeKeysKey(), JSON.stringify(doc || {})]).then(function () {
        var message = JSON.stringify({node_id: self.nodeId});
        return self.client.sendCommand(['PUBLISH', self._e2eeKeysChannel(), message]);
    });
};

/**
 * Invoke `callback()` whenever a peer updates the shared key directory.
 * Updates published by this node are ignored.
 */
ClusterSync.prototype.onE2eeKeysChange = function (callback) {
    var self = this;
    return this.subscriber.subscribe(this._e2eeKeysChannel(), function (raw) {
        var message;
        try {
            message = JSON.parse(raw);
        }
