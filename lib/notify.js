...
function validateActions (raw) {
    ...
    for (var i = 0; i < raw.length; i++) {
        ...
        if (a.action === 'http') {
            if (a.method !== undefined && typeof a.method !== 'string')
                return { error: 'action ' + i + ': method must be a string' };
            var method = a.method === undefined ? 'POST' : a.method.toUpperCase();
            ...
            if (a.headers !== undefined) {
                ...
                for (var k = 0; k < keys.length; k++) {
                    // Same prototype-pollution posture as isUnsafePubKey on
                    // realm/topic keys — and fail-loud rather than the silent
                    // drop a plain-object assignment of __proto__ would cause.
                    if (keys[k] === '__proto__' || keys[k] === 'constructor' || keys[k] === 'prototype')
                        return { error: 'action ' + i + ': unsafe header name' };
...
// Normalize a client-supplied message time to unix SECONDS. Accepts a plausible
// seconds value (year ~2001–2096); rejects garbage AND millisecond values (so a
// Date.now() sent by mistake can't land a card in the year 56000), falling back
// to the broker's receipt time.
function normalizeTime (t) {
    var n = Math.floor(Number(t));
    if (Number.isFinite(n) && n > 1000000000 && n < 4000000000) return n;
