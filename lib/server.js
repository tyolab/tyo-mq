     ...
     this.create = function (io) {
        ...
        io.sockets.on('connection', function(socket) {
            ...
            socket.on('PRODUCE', function (msg) {
                ...
                // var event = eventManager.toConsumerEvent(obj.event, producerName);
                var event = obj.event;
            ...
            socket.on('CONSUMER', function (consumer) {
                ...
                if (existingMeta && existingMeta.socket && existingMeta.socket !== socket.id) {
                    ...
                    if (existingSocket) {
                        server.logger.error("Duplicate consumer name '" + consumer.name + "': already registered with socket " + existingMeta.socket + ". Rejecting new socket " + socket.id);
            ...
            socket.on('PING', function (msg, callback) {
                console.log("received PING message: " + JSON.stringify(msg));
                if (callback) {
            ...
            socket.on('PRODUCER', function (msg) {
                ...
                for (var id in ids) {
                    ...
                    server.logger.log("Triggering event: " + event);
                    server.logger.log("Producer is online (" + producerName + ")");
                    if (consumerMeta.socket)
