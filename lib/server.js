     ...
     this.create = function (io) {
        ...
        io.sockets.on('connection', function(socket) {
            ...
            function sendConsumeMessage (socketId, event, message, producer, delivery) {
                incMetric('tyo_mq_messages_delivered_total', {realm: currentRealmId(), event: String(event)});
            ...
            function deadLetterMessage(msgId, reason) {
                incMetric('tyo_mq_messages_dlq_total', {realm: currentRealmId()});
            ...
            function scheduleAckTimeout(entry, subscription, consumerMeta, attempt, policy) {
                pendingAcks[entry.id] = {
                    ...
                    timer: setTimeout(function () {
                        ...
                        incMetric('tyo_mq_ack_timeout_total', {realm: entry.realm || currentRealmId()});
            ...
            function broadcastToGroup (event, message, producer, group) {
                var msgObj = {event: event, message: message, from: producer};
                ...
                for (var eventStr in realm.subscriptions) {
                    ...
                    for (var consumer in subscriptions) {
                        ...
                        var consumerMeta = getConsumerMetaInfo(currentRealmId(), subscription.name);
                        if (!consumerMeta.socket)
                            continue;
            ...
            function handleProduce(obj) {
                ...
                if (obj.method && obj.method === 'broadcast') {
                    if (obj.broadcast === 'group' && obj.group)
                        broadcastToGroup(event, message, producerName, obj.group);
                    else
