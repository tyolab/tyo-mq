...
function Server(options) {
    ...
    this.reloadSettings = function () {
     ...
     this.create = function (io) {
        ...
        // Give the FCM transport a logger for its token-free auth-failure warn
        // (401/403 => OUR creds are wrong; the device token is NOT pruned).
        if (pushConfig && pushConfig.transportName === 'fcm' && pushConfig.transport)
            ...
            onAuthorized(Push.PUSH_REGISTER_EVENT, ['producer', 'consumer'], function (msg, callback) {
                ...
                var finishRegister = function () {
                    pushRegistry.register(realmId, identity, {
                        transport: transport, token: token, app_id: appId || null, now: Date.now()
                    });
                    if (clusterSync)
