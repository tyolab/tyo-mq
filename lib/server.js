...
function Server(options) {
    ...
    var validateExternalToken = function (token, url, realm) {
        return new Promise(function (resolve, reject) {
            ...
            var authSecret = (server.authOptions || {}).auth_secret;
            if (authSecret)
                headers['x-mq-auth-secret'] = authSecret;
    ...
    var validateToken = function (token, desiredRealm) {
        ...
        if (auth.auth_url)
            return validateExternalToken(token, auth.auth_url, desiredRealm);
