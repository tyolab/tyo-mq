     ...
     this.create = function (io) {
        ...
        io.sockets.on('connection', function(socket) {
            ...
            function handleProduce(obj) {
                ...
                if (getRealmE2eePolicy(currentRealmId()) === 'required' && !obj.enc) {
                    server.logger.warn("PRODUCE rejected: realm '" + currentRealmId() + "' requires E2EE and no 'enc' envelope was present (producer='" + producerName + "', event='" + event + "')");
                    sendErrorMessage({message: "Realm '" + currentRealmId() + "' requires end-to-end encrypted messages", code: -6});
                    return;
                }
                ...
                if (obj.method && obj.method === 'broadcast') {
                    if (obj.broadcast === 'group' && obj.group)
                        broadcastToGroup(currentRealmId(), event, message, producerName, obj.group, {enc: obj.enc, to: obj.to});
                    else
                        broadcastConsumeMessage(currentRealmId(), event, message, producerName, {enc: obj.enc, to: obj.to});
                }
                else
                    generateMessage(currentRealmId(), event, message, producerName, {ttl: ttl, guaranteed: guaranteed, enc: obj.enc, to: obj.to});
