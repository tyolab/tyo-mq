     ...
     this.create = function (io) {
        ...
        io.sockets.on('connection', function(socket) {
            ...
            function subscribeMessage (event, producer, consumer) {
                ...
                sendSubscriptionMessageByName(eventStr, producer, consumer, consumerMeta.id, true);
            }
            ...
            function sendSubscriptionMessageToProducer(eventStr, producerMeta, consumer, consumerId, status) {
                if (producerMeta.online) {
                    ...
                    sendSubscriptionMessageWithConsumerInfo(producerMeta.socket.id, producerMeta.id, [eventStr], producerMeta.name, consumer, consumerId, status);
            ...
            function sendSubscriptionMessage(events, producer, consumer, consumerId, status) {
                ...
                if (producerMeta && producerMeta.id)
                    sendSubscriptionMessageWithConsumerInfo(producerMeta.socket.id, producerMeta.id, producerMeta.id, events, producer, consumer, consumerId, status);
            }

            // it seems the new updates weren't pushed to the remote repo
            function sendSubscriptionMessageWithConsumerInfo(socketId, id, events, producer, consumer, consumerId, status) {
                if (status === null)
                    status = true;

                var onSubscribeEvent = eventManager.toOnSubscribeEvent(id);

                sendMessage(socketId, onSubscribeEvent, {name:consumer, id:consumerId, events:events, online:status});
            }
            ...
            socket.on('CONSUMER', function (consumer) {
                ...
                consumerMeta.id = consumer.id || socket.id;
            ...
            socket.on('disconnect', function () {
                ...
                event = eventManager.toOnDisconnectEvent(id);
