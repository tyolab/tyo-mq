    ...
    this.reloadSettings = function () {
     ...
     this.create = function (io) {
        ...
        io.sockets.on('connection', function(socket) {
            ...
            function enqueueDurableMessage(realmId, event, message, producer, subscription, ttl) {
                if (!subscription || !subscription.durable)
                    return;
            ...
            function replayDurableMessages(realmId, event, subscription, consumerMeta) {
                if (!subscription || !subscription.durable || !consumerMeta || !consumerMeta.socket)
                    return;
                            ...
                            for (var i = 0; i < ar.length; ++i) {
                                ...
                                if (name === Constants.ALL_PUBLISHERS || name === producer) {
                                    if (consumerMeta.socket)
                                        ...
                                        enqueueDurableMessage(targetRealmId, eventStr, msgObj, producer, subscription, options.ttl);
                                    break;
                                }
                            }
                        }
            ...
            function handleProduce(obj) {
                ...
                if (obj.method && obj.method === 'broadcast')
                    ...
                    generateMessage(event, message, producerName, {ttl: ttl});
            }
