     ...
     this.create = function (io) {
        ...
        io.sockets.on('connection', function(socket) {
            ...
            onAuthorized('PREKEY_PUBLISH', ['producer', 'consumer'], function (msg, callback) {
                ...
                if (!prekeyFieldOk(obj.identity_key) || !prekeyFieldOk(obj.signed_prekey) || !prekeyFieldOk(obj.signed_prekey_sig)) {
                    if (callback) callback({ok: false, code: 400, message: 'identity_key, signed_prekey and signed_prekey_sig are required (base64, <= ' + Constants.E2EE_MAX_PUBLIC_KEY_LENGTH + ' chars)'});
                    return;
                }
                ...
                for (var i = 0; i < oneTime.length; i++) {
                    if (!prekeyFieldOk(oneTime[i])) {
                        if (callback) callback({ok: false, code: 400, message: 'each one-time prekey must be base64, <= ' + Constants.E2EE_MAX_PUBLIC_KEY_LENGTH + ' chars'});
                        return;
                    }
                }
                ...
                // Static parts are replaced (identity/signed-prekey rotation);
                // one-time prekeys are APPENDED to the pool (replenish), capped.
                ...
                bundle.one_time = bundle.one_time.concat(oneTime).slice(0, Constants.PREKEY_POOL_MAX);
