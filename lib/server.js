...
function Server(options) {
    ...
    // ── temporary (disposable) realms ─────────────────────────────────────
    // A realm is created in one of two forms: permanent (the default — lives
    // until an explicit remove_realm) or temporary, tagged with an expiry.
    // Expired temporary realms are disposed of automatically: config, scoped
    // tokens, runtime state, stored messages, and connected sockets all go.

    var TEMPORARY_REALM_DEFAULT_TTL_MS = 24 * 60 * 60 * 1000; // 1 day
    ...
    var getTemporaryRealmDefaultTtlMs = function () {
        var auth = server.authOptions || {};
        return parseRealmTtlMs(auth.temporary_realm_ttl || auth.temporaryRealmTtl)
            || TEMPORARY_REALM_DEFAULT_TTL_MS;
    };
    ...
    // manager_key_configured, temporary, expires_at}. New realms accept
    // {temporary: true, ttl: '2h'} to create the disposable form; rotating an
    ...
    var handleCreateRealmRequest = function (req, res) {
        ...
        readJsonBody(req, 64 * 1024, function (err, body) {
            ...
            // New realms come in two forms: permanent (default) or temporary/
            // disposable — auto-disposed once the ttl lapses.
            if (body.temporary || body.disposable)
                createBody.temporary = true;
            ...
            sendJson(res, 200, {
                ...
                temporary: !!realmConfig.temporary,
    ...
    var applyAuthManagementCommand = function (body) {
        ...
        if (command === 'set_global_auth') {
            ...
            // A realm is either permanent (default) or temporary/disposable:
            // tagged with an expiry and disposed of automatically when it
            // lapses. Passing a ttl alone implies temporary.
            if (body.temporary || body.disposable || body.ttl !== undefined) {
                var ttlMs = body.ttl !== undefined
                    ? parseRealmTtlMs(body.ttl)
                    : getTemporaryRealmDefaultTtlMs();
                if (!ttlMs)
                    return {ok: false, code: 400, message: 'invalid ttl — use e.g. 300, "30m", "2h", "7d"'};
                auth.realms[body.realm].temporary = true;
            ...
            // Convert between the two realm forms: {temporary: false} makes a
            // disposable realm permanent; {temporary: true, ttl} (or a bare
            ...
            if (!auth.realms[body.realm])
                return {ok: false, code: 404, message: 'realm not found'};
            var wantTemporary = body.temporary !== undefined ? !!body.temporary : true;
            if (!wantTemporary) {
    ...
    // ── temporary-realm disposal sweep ────────────────────────────────────
    // Disposes every temporary realm whose expiry has lapsed, through the same
    ...
    var sweepTemporaryRealms = function () {
        var auth = server.authOptions || {};
        ...
        Object.keys(realmConfigs).forEach(function (realmId) {
            ...
            if (!rc.temporary || !rc.expires_at || rc.expires_at > now)
                return;
            var result = applyAuthManagementCommand({command: 'remove_realm', realm: realmId});
            if (result && result.ok) {
                disposed++;
                if (server.logger && server.logger.info)
                    server.logger.info("Temporary realm '" + realmId + "' expired — disposed"
                ...
                server.logger.error("Temporary realm '" + realmId + "' disposal failed: "
    ...
    var startTemporaryRealmSweep = function () {
        if (temporaryRealmTimer)
            return;
        temporaryRealmTimer = setInterval(function () {
            try { sweepTemporaryRealms(); }
            catch (err) {
                if (server.logger && server.logger.error)
                    server.logger.error("Temporary realm sweep failed: " + (err && err.message || err));
            }
        }, 60000);
        if (temporaryRealmTimer.unref)
            temporaryRealmTimer.unref(); // never keep the process alive on our account
    };

    // Public: stop the sweep timer (shutdown / tests).
    this.stopTemporaryRealmSweep = function () {
        if (temporaryRealmTimer) {
            clearInterval(temporaryRealmTimer);
            temporaryRealmTimer = null;
        }
        return this;
    };

    // Public: run a single disposal pass immediately; returns the count disposed.
    this.sweepTemporaryRealms = function () {
        return sweepTemporaryRealms();
    };
    ...
    this.reloadSettings = function () {
     ...
     this.create = function (io) {
        ...
        currentIo = io;
        startIdleEviction();
        sweepTemporaryRealms();
        startTemporaryRealmSweep();
