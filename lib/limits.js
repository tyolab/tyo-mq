 ...
 *     "max_queued_per_recipient": 1000,       // sealed inbox depth per identity
 *                                             // (defaults to 1000 when limits
 *                                             // are enabled; 0 disables)
 ...
 * operator enables exactly what they need. The only exception is
 * `max_queued_per_recipient`, which defaults to 1000 when limits are enabled
 * (a sealed inbox is attacker-fillable by anonymous senders in unrestricted
 * mode, so it must not default to unbounded); set it to 0 to disable.
 */
...
// Keys with a non-Infinity default when limits are enabled but the key is
// absent. An explicit 0/negative value still means "unlimited".
var DEFAULTS = {
    max_queued_per_recipient: 1000
};
...
Limits.prototype.value = function (realm, name) {
    ...
    if (realm && overrides[realm] && overrides[realm][name] !== undefined)
        ...
        raw = config[name];
    if (raw === undefined && Object.prototype.hasOwnProperty.call(DEFAULTS, name))
        raw = DEFAULTS[name];
    raw = Number(raw);
