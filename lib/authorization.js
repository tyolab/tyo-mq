...
function signedManagerPayload(managerToken, action, body) {
    body = body || {};
    return {
        body: body,
        proof: adminSignature.createAdminProof(managerToken, action, body)
    };
}

function nextAuthorizationRequest(managerToken, filter, options) {
    return withSocket(options, function (socket) {
        return emitAck(socket, 'AUTHORIZATION_NEXT',
            signedManagerPayload(managerToken, 'AUTHORIZATION_NEXT', filter || {}),
            options && options.timeout);
    });
}

function decideAuthorizationRequest(managerToken, decision, options) {
    return withSocket(options, function (socket) {
        return emitAck(socket, 'AUTHORIZATION_DECIDE',
            signedManagerPayload(managerToken, 'AUTHORIZATION_DECIDE', decision || {}),
            options && options.timeout);
    });
}
...
function nextRealmAuthorizationRequest(managerKey, realm, options) {
    return nextAuthorizationRequest(managerKey, {realm: realm}, options);
}

function decideRealmAuthorizationRequest(managerKey, decision, options) {
    return decideAuthorizationRequest(managerKey, decision, options);
}
...
module.exports = {
    ...
    decideRealmAuthorizationRequest: decideRealmAuthorizationRequest,
    nextAuthorizationRequest: nextAuthorizationRequest,
    nextRealmAuthorizationRequest: nextRealmAuthorizationRequest,
